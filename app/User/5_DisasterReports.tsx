...
//   const [idvalue,setvalue]=useState<string>('');
...
//     const parameter=idvalue;
//     router.push(`./Userpage6?param=${parameter}`); // Change './NextPage' to the actual path of your next page
//     handleCloseModal();
//   };
...
const RealTimeChecker = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(firestore, 'DisasterReports'), (snapshot) => {
      const newData = snapshot.docs.map((doc) => {
        ...
        const formattedTimestamp = timestamp ? moment(timestamp).format('MMM D, YYYY h:mm A') : 'Unknown Time';
        return {
  ...
  const handlePress = (report: TestData) => {
    setSelectedReport(report);
    setvalue(report.id); 
    //  console.log(report)                     // Set the idvalue to the selected report's id
    setModalVisible(true);
  };
  ...
  const handleNavigate = (report:TestData) => {
    const parameter = report.id;
    // console.log(parameter)
    router.push(`./6_ShelfLife?param=${parameter}`);
    handleCloseModal();
  };
